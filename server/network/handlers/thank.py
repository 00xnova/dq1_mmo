"""Thank / ty private ack (extracted). Multiplayer social, not a party.

Uses private_social_delivery so failed sends refund chat and restore AFK.
Echo includes near/far peer context without map coords — pairs with /share.
"""
...
THANK_TYPES = frozenset(
    {"thank", "thanks", "ty", "thx", "thankyou", "thank_you"}
)
ALL_TYPES = THANK_TYPES
...
async def handle(
    ...
    """Dispatch thank/ty. Returns None if not a thank message."""
    ...
    echo = dict(thank_msg)
    echo["message"] = f"You thanked {tname}{suffix}."
    if peer:
        if "nearby" in peer:
