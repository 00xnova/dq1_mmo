"""v0.5.137: thank WS · near/far message · version."""
...
def test_thank_ws(tmp_path, monkeypatch):
    ...
    try:
        ta, ca = register_char(base, "tha@ex.com", "Ta", "ThankA")
        tb, cb = register_char(base, "thb@ex.com", "Tb", "ThankB")
        ...
        async def flow():
            ...
            async with websockets.connect(ws_url) as wsa, websockets.connect(
                ...
                await wsa.send(json.dumps({"type": "thank", "to": "ThankB"}))
                ok = await recv_until(wsa, "thank", "error")
                assert ok.get("type") == "thank", ok
                assert "You thanked" in (ok.get("message") or "")
                assert "nearby_count" in ok
                peer = await recv_until(wsb, "thank", "error")
                assert peer.get("type") == "thank"
                assert "thanks" in (peer.get("message") or "").lower() or peer.get(
                    "from"
                )
